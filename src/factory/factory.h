...
class Factory
{
    ...
    /// Removes a class wrapper that was previously added to the factory.
    void RemoveWrapper( const char* name );
    ...
    /// Returns the number of objects created by the factory.
...
template < typename TBase, typename Argument >
void Factory< TBase, Argument >::RemoveWrapper( const char* name )
{
    auto iterator = mWrapperMap.find( name );
    if ( iterator == mWrapperMap.end() )
        return;
    mWrapperMap.erase( iterator );
    mObjectVectorMap.erase( mObjectVectorMap.find( name ) );
}
...
boost::shared_ptr< TBase > Factory< TBase, Argument >::CreateInstance( const xmlparser::XmlParameter* param, const Argument* arg )
{
    ...
    if ( param->HasElementAttribute( "cacheref" ) )
        return GetCachedInstance( param->GetElementAttributeCharPtrValue( "cacheref" ) );
...
template < typename TBase, typename Argument >
size_t Factory< TBase, Argument >::GetNumberOfObjects() const
{
