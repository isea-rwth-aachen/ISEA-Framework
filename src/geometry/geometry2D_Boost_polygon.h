...
public:
    ...
    ///Returns true of this instance and rhs are tangent by tolerance, returns false if they are not tangent, undefinined behaviour if they overlap
    bool IsAdjacentTo(const Geometry2D<T> &rhs, const Tolerance<T> tolerance) const;
    ///Return bounding box of this instance: the smallest possible box that surrounds this polygon and whose edges are parallel to coordinate axis
    ...
    for(size_t i = 0; i < vertices.size(); ++i)
    {
        ...
        if(arcs.count(i) == 1)
        {
            ...
            for(size_t i = 0; i < n; ++i)
                polygonVertices.push_back(CreatePointFromT(arc.Get1() + radius * cos(phi_i.GetRad() + (0.5 + i) * dPhi.GetRad())
...
template<typename T>
bool Geometry2D<T>::IsPointCloseToLine(const Point &point, const Point &lineStart, const Point &lineEnd, const Tolerance<T> tolerance) const
{
