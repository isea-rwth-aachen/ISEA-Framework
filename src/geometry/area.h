...
class Area : public Plane< T >
{
     ...
     */
    Area( const vector< TwoDim< T > > &vertices, const Plane< T > &plane, T distanceToGridVertex, T conductivity, T emissivity,
...
Area< T >::Area( const vector< TwoDim< T > > &vertices, const Plane< T > &plane, T distanceToGridVertex, T conductivity,
                 T emissivity, Location location, const map< size_t, TwoDim< T > > &arcs, T arcPolygonEdgesLength )
    : Plane< T >( plane )
    , mVertices( vertices )
    , mArcs( arcs )
