...
///CylindricElement is a ThermalElement whose edges are parallel to the vector of a cylindric coordinate system, i.e. a cylinder segment
template<typename T = double>
class CylindricElement: public ThermalElement<T>
{
public:
    /**
     * @param[in] emplacementPoint Translation vector between global cartesian and cylindric coordinate system used in this instance
     ...
     */
    CylindricElement(const Cartesian<T> &emplacementPoint
                   , const Cylindric<T> &startVector
...
template<typename T>
CylindricElement<T>::CylindricElement(const Cartesian<T> &emplacementPoint
                                    , const Cylindric<T> &startVector
                                    , const Cylindric<T> &deltaVector
                                    , T initialTemperatureValue
                                    , const Material<T> *material
    ...
    , mDeltaVector(deltaVector)
{
    ...
    ThermalElement<T>::SetElement(initialTemperatureValue
                                , material
                                , CylToCart<T>(emplacementPoint, startVector + deltaVector / 2.0)
                                , MakeVolume()
                                , thermalState
                                , thermalStateFactor);
}
