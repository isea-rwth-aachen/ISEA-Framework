...
///TriangularElement is a ThermalElement with a triangular base in a plane parallel to the xy-plane and a height in z-direction
...
public:
     ...
     * @param[in] vertex1,vertex2,vertex3 Vertices 1-3 define the base triangle of the triangular element as two dimensional coordinates in a plane parallel to the xy-plane with the emplacement point as origin
     * @param[in] height Extension in z-direction
     */
    TriangularElement(const Cartesian<T> &emplacementPoint
                    , const TwoDim<T> &vertex1
                    , const TwoDim<T> &vertex2
                    , const TwoDim<T> &vertex3
                    , T height
...
template<typename T>
TriangularElement<T>::TriangularElement(const Cartesian<T> &emplacementPoint
                                      , const TwoDim<T> &vertex1
                                      , const TwoDim<T> &vertex2
                                      , const TwoDim<T> &vertex3
                                      , T height
    ...
    , mHeight(height)
{
    ...
    ThermalElement<T>::SetElement(initialTemperatureValue
                                , material
                                , emplacementPoint + Cartesian<T>((vertex1.Get1() + vertex2.Get1() + vertex3.Get1()) / 3.0
                                                                , (vertex1.Get2() + vertex2.Get2() + vertex3.Get2()) / 3.0
                                                                , height / 2.0)
                                , height * 0.5 * fabs(( vertex1.Get2() + vertex2.Get2() ) * ( vertex1.Get1() - vertex2.Get1() )
...
bool TriangularElement<T>::CheckForLineError(const TwoDim<T> &vertex1, const TwoDim<T> &vertex2, const TwoDim<T> &vertex3) const
{
    ...
    if(vertex1.DistanceTo(vertex2) == 0.0
    || vertex1.DistanceTo(vertex3) == 0.0
    || vertex2.DistanceTo(vertex3) == 0.0)
        return true;

    //All three points on a line
    TwoDim<T> deltaVertex = vertex1 - vertex2;
    if( (vertex3.Get1() - vertex1.Get1()) / (vertex2.Get1() - vertex1.Get1()) == (vertex3.Get2() - vertex1.Get2()) / (vertex2.Get2() - vertex1.Get2()))
        return true;
