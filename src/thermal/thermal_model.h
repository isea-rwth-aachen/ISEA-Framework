...
class ThermalModel
{
    ...
    T GetTotalHeatCapacity() const
    {
        T totalHeatCap = 0;
        for ( auto &j : mBlocks )
        {
            for ( auto &i : j.mThermalElements )
            {
                totalHeatCap += i->GetVolume() * i->GetMaterial()->GetDensity() * i->GetMaterial()->GetSpecificCapacity();
            }
        }
        return totalHeatCap;
    };
    T ConvectionArea() const { return mConvectionArea; };
 ...
 vector< CooledAreaData > &cooledAreaDataVector, vector< vector< IndexedValue< T > > > &conductivityMatrix )
{
    ...
    for ( size_t i = 0; i < mBlocks.size(); ++i )
    {
        ...
        {
            const T areaTolerance = area.mArea.GetAreaValue() * mTolerance.mPercentOfQuantity / 100.0;


            if ( area.mAreaNotCovered < -areaTolerance )
            {
                std::cout << mBlocks[i].mVolumeName << std::endl;
                std::cout << area.mArea.GetAreaValue() << std::endl;
                // std::string name = CxxTest::ValueTraits< misc::StrCont >( mBlocks[i].mVolumeName ).asString();
                ErrorFunction< std::runtime_error >( __FUNCTION__, __LINE__, __FILE__, "OverlappingAreaBiggerActualOne",
                                                     areaTolerance, "Unknown" );
            }
            ...
            if ( area.mAreaNotCovered > areaTolerance )
            {
                ...
                mConvectionArea += area.mAreaNotCovered;
                                           ...
                                           vector< vector< TaylorData< T > > > &dirichletDataVector )
{
    ...
    BOOST_FOREACH ( const vector< const ExternalAreaData * > &areasOnTheSamePlane, areasSortedBySamePlane )
    {
        ...
        BOOST_FOREACH ( const ExternalAreaData *externalAreaData, areasOnTheSamePlane )
        {
            ...
            data.mCooling = 0;
            data.mEmissivity = externalAreaData->mAreaPointer->GetEmissivity();
