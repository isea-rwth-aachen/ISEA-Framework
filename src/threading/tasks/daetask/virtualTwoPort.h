...
struct RowCopyInfo
{
    size_t FirstRowToCopy;
    size_t LastRowToCopy;
};
...
template<typename MatrixType>
std::vector<boost::shared_ptr<electrical::TwoPort<MatrixType> > > & VirtualTwoPort< MPICommunicator, MatrixType >::GetObservableTwoPorts()
{
...
void VirtualTwoPort< MPICommunicator, MatrixType >::RegisterObservableTwoPort(boost::shared_ptr<electrical::TwoPort<MatrixType> > tp)
{
    ...
    boost::shared_ptr<electrical::TwoPortWithChild<MatrixType> > tpwc( boost::static_pointer_cast< electrical::TwoPortWithChild<MatrixType> >(tp) );
