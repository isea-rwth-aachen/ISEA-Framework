...
//#endif
    ...
    }
    {
        ...
        testStream << 1.0 << ", " << 4.5 << "\n";
        testStream << 1.1 << ", " << -6.3 << "\n";
        testStream << "# comment with windows line ending\r\n";
        testStream << 2.0 << ", " << -2.0 << "\n";
        testStream << 2.0 << ", " << -2.0 << "\n";
        testStream << 3.3 << ", " << 1.0 << "\n";
...
void TestTimeSeries::TestHeaderParsing()
{
    typedef electrical::TimeSeries< double, electrical::EvalNoInterpolation > Profile;
    {
        std::stringstream stream;
        stream << "# type=POweR\n0, 0";
        Profile profile( stream );
        TS_ASSERT_EQUALS( profile.GetType(), electrical::TimeSeriesType::POWER );
    }
    {
        std::stringstream stream;
        stream << "#type=currEnT;\n0, 0";
        Profile profile( stream );
        TS_ASSERT_EQUALS( profile.GetType(), electrical::TimeSeriesType::CURRENT );
    }
    {
        std::stringstream stream;
        stream << "# comment\n0,0";
    ...
    }
    {
        std::stringstream stream;
        stream << "# type=voltage,\n0,0";
        TS_ASSERT_THROWS_EQUALS( Profile profile( stream ), const std::runtime_error &e,
                                 strcmp( e.what(), "The profile type voltage is unknown.\n" ), 0 );
    }
    {
        std::stringstream stream;
        stream << "# type=power\r\n0.0, 0.0";
