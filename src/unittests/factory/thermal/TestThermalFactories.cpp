...
void TestFactories::TestCoolingFactory()
{
    ...
    factory::Factory< Cooling<> > *coolingFactory = factoryBuilder.BuildCoolingFactory();
...
void TestFactories::TestCoolingBlockFactory()
{
    ...
    xmlparser::tinyxml2::XmlParserImpl parser;
    factory::FactoryBuilder< myMatrixType, ScalarUnit > factoryBuilder;
    factory::Factory< CoolingBlock<>, factory::ArgumentTypeCoolingBlock< double > > *coolingBlockFactory =
     factoryBuilder.BuildCoolingBlockFactory();
...
void TestFactories::TestMaterialFactory()
{
    const char *xmlMaterial =
     "\
        <ThermalMaterials>\
            ...
            <Fail2 class=\"Material\">\
                <Density>1.0</Density>\
                <SpecificCapacity>1.0</SpecificCapacity>\
    ...
    factory::FactoryBuilder< myMatrixType, ScalarUnit > factoryBuilder;
    factory::Factory< Material<> > *materialFactory = factoryBuilder.BuildMaterialFactory();
    ...
    TS_ASSERT_DELTA( copper->GetEmissivity(), 0.1, 0.000001 );
    ...
    TS_ASSERT_DELTA( iron->GetEmissivity(), 0.2, 0.000001 );
    ...
    TS_ASSERT_DELTA( rubber->GetEmissivity(), 0.9, 0.000001 );

    //#if defined(__EXCEPTIONS__)
    TS_ASSERT_THROWS_ANYTHING( materialFactory->CreateInstance( materialsParam->GetElementChild( "Fail" ) ) );
    TS_ASSERT_THROWS_ANYTHING( materialFactory->CreateInstance( materialsParam->GetElementChild( "Fail2" ) ) );
    //#endif
}
...
void TestFactories::TestBlockFactory()
{
    ...
    factory::FactoryBuilder< myMatrixType, ScalarUnit > factoryBuilder;
    factory::Factory< ThermalBlock<>, factory::ArgumentTypeThermalBlock< double > > *blockFactory =
     factoryBuilder.BuildBlockFactory();
    factory::Factory< thermal::Material< double > > *materialFactory = factoryBuilder.BuildMaterialFactory();
...
void TestFactories::TestThermalFactory()
{
    ...
    factory::FactoryBuilder< myMatrixType, ScalarUnit > factoryBuilder;
    ThermalFactory<> *thermalFactory = factoryBuilder.BuildThermalFactory();
